...
Function LaunchNpp
  ...
  Exec '"$WINDIR\explorer.exe" "$INSTDIR\TeeJ-editor.exe"'
  ...
  ${ForEach} $R1 1 5 + 1				; Loop to find opened Npp instance
	...
	IntCmp $R0 0 NotYetExecuted
		...
		Exec '"$INSTDIR\TeeJ-editor.exe" "$INSTDIR\change.log" '
		${Break}
	NotYetExecuted:
		Sleep 1000
  ${Next}
...
!macro CheckIfRunning un
	Function ${un}CheckIfRunning
		...
		IntCmp $R0 0 NotRunning
			...
			System::Call 'kernel32::CloseHandle(i $R0)'
			MessageBox MB_RETRYCANCEL|MB_DEFBUTTON1|MB_ICONSTOP "Cannot continue the installation: TeeJ-editor is running.\
			          $\n$\n\
                      Please close TeeJ-editor, then click ''Retry''." IDRETRY Retry IDCANCEL Cancel
			Retry:
				Goto Check
...
Function ExtraOptions
	...
	${NSD_CreateCheckbox} 0 80 100% 30u "Don't use %APPDATA%$\nEnable this option to make TeeJ-editor load/write the configuration files from/to its install directory. Check it if you use TeeJ-editor in a USB device."
...
Function checkCompatibility
	...
	StrCmp $WinVer "95" 0 +3
		MessageBox MB_OK|MB_ICONSTOP "TeeJ-editor does not support your OS. The installation will be aborted."
		Abort
...
!ifdef ARCHARM64
	...
	${Else}
		...
		MessageBox MB_YESNO|MB_ICONSTOP "This installer contains ARM64 version of TeeJ-editor incompatible with your computer processor running, so the installation will be aborted.$\n$\nDo you want to go to the Notepad++ site to download a compatible (x86/x64) installer instead?" IDYES arm64_openDlPage IDNO arm64_goQuit
arm64_openDlPage:
...
Function writeInstallInfoInRegistry
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\TeeJ-editor.exe" "" "$INSTDIR\TeeJ-editor.exe"
	...
	!endif
	WriteRegStr HKLM "${UNINSTALL_REG_KEY}" "Publisher" "TeeJ"
	...
	WriteRegStr HKLM "${UNINSTALL_REG_KEY}" "DisplayIcon" "$INSTDIR\TeeJ-editor.exe"
...
!define RUNPROC_WND_CLASS "TeeJ-editor"
