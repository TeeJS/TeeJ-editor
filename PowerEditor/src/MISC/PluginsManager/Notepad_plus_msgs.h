...
#define NPPMSG  (WM_USER + 1000)
	...
	// For each created dialog in your plugin, you should register it (and unregister while destroy it) to TeeJ-editor by using this message.
	...
	// Saves the current opened files in TeeJ-editor as a group of files (session) as an xml file.
	// wParam: 0 (not used)
	...
	// A plugin can create a Scintilla for its usage by sending this message to TeeJ-editor.
	// wParam: 0 (not used)
	// lParam[in]: hParent - If set (non NULL), it will be the parent window of this created Scintilla handle, otherwise the parent window is TeeJ-editor
	...
	// TeeJ-editor will deallocate every created Scintilla control on exit, this message returns TRUE but does nothing.
	// wParam: 0 (not used)
	...
	// lParam: 0 (not used)
	// Return: menu handle (HMENU) of choice (plugin menu handle or TeeJ-editor main menu handle)
	...
	// wParam[in]: if dir2Search is not NULL, it will be set as working directory in which TeeJ-editor will search
	...
	// Pass the necessary dockingData to TeeJ-editor in order to make your dialog dockable.
	// wParam: 0 (not used)
	...
	// Open all files of same session in TeeJ-editor via a xml format session file sessionFileName.
	// wParam: 0 (not used)
	...
	// The returned value is TRUE if TeeJ-editor found the plugin by its module name (destModuleName), and pass the info (communicationInfo) to the module.
	...
	// Run TeeJ-editor command with the given command ID.
	// wParam: 0 (not used)
	// lParam[in]: cmdID - See "menuCmdID.h" for all the TeeJ-editor menu command items
	// Return TRUE
	...
	// If fullPathName2Open has been already opened in TeeJ-editor, the it will be activated and becomes the current document.
	// wParam: 0 (not used)
	...
	// Get the code page associated with the current localisation of TeeJ-editor.
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	...
	// then menu item ID 46581, 46582, 46583 and 46584 are preserved by TeeJ-editor, and they are safe to be used by the plugin.

	#define NPPM_ALLOCATEMARKER  (NPPMSG + 82)
    // BOOL NPPM_ALLOCATEMARKER(int numberRequested, int* startNumber)
	// Allocate a number of consecutive marker IDs to a plugin: if a plugin need to add a marker on TeeJ-editor's Scintilla marker margin,
	...
	// then marker ID 16, 17 and 18 are preserved by TeeJ-editor, and they are safe to be used by the plugin.
	...
	// Check to see if loading plugins from "%APPDATA%\..\Local\TeeJ-editor\plugins" is allowed.
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	...
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	// Return: current edit view of TeeJ-editor. Only 2 possible values: 0 = Main, 1 = Secondary
	...
	// Save the file (opened in TeeJ-editor) with the given full file name path.
	// wParam: 0 (not used)
	...
	// Disable TeeJ-editor auto-update.
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	// Return TRUE
	...
	// It may help some plugins to disable non-dynamic line number margins width to have a smoothly visual effect while vertical scrolling the content in TeeJ-editor
	// wParam: 0 (not used)
	...
	//              - Standard (0) means TeeJ-editor will keep the same TAB indentation between lines;
	//              - C_Like (1) means TeeJ-editor will perform a C-Language style indentation for the selected external language;
	...
	// Get the current use Auto-Indentation setting in TeeJ-editor Preferences.
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	...
	// Get TeeJ-editor Dark Mode status (ON or OFF).
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	...
	// then indicator ID 7 is preserved by TeeJ-editor, and it is safe to be used by the plugin.
	...
	// By default, TeeJ-editor only forwards SCN_MODIFIED with the following 5 flags/events:
	// SC_MOD_DELETETEXT | SC_MOD_INSERTTEXT | SC_PERFORMED_UNDO | SC_PERFORMED_REDO | SC_MOD_CHANGEINDICATOR to plugins.
	// If your plugin needs to process other SCN_MODIFIED events, you should add the required flags by sending this message to TeeJ-editor. You can send it immediately after receiving NPPN_READY,
	// or only when your plugin needs to listen to specific events (to avoid penalizing TeeJ-editor's performance). Just ensure that the message is sent only once.
	// wParam: 0 (not used)
	...
	// Get TeeJ-editor toolbar icon set choice (Fluent UI: small, Fluent UI: large, Filled Fluent UI: small, Filled Fluent UI: large and Standard icons: small.
	// wParam: 0 (not used)
	// lParam: 0 (not used)
	...
	// Get path for the active TeeJ-editor settings: it will use -settingsDir path if that's defined; if not, it will use Cloud directory if that's defined;
	...
	// where the active TeeJ-editor settings are stored, whichever location they are currently set to.
	...
	// 
	// Note: This message is for the active TeeJ-editor configuration location.  If you are looking for the settings directory for plugins (...\Plugins\Config\),
...
#define NPPN_FIRST 1000
	#define NPPN_READY (NPPN_FIRST + 1) // To notify plugins that all the initialization for launching TeeJ-editor is complete.
	...
	#define NPPN_SHUTDOWN (NPPN_FIRST + 9) // To notify plugins that TeeJ-editor is about to be shutdowned.
	...
	#define NPPN_BEFORESHUTDOWN (NPPN_FIRST + 19)  // To notify plugins that TeeJ-editor shutdown has been triggered, files have not been closed yet
	//scnNotification->nmhdr.code = NPPN_BEFORESHUTDOWN;
	//scnNotification->nmhdr.hwndFrom = hwndNpp;
	//scnNotification->nmhdr.idFrom = 0;

	#define NPPN_CANCELSHUTDOWN (NPPN_FIRST + 20)  // To notify plugins that TeeJ-editor shutdown has been cancelled
	...
	// -pluginMessage="NppExecScriptPath=C:\Program Files\TeeJ-editor\plugins\NppExec\init.py;NppExecArg2=arg2Value;mimeToolsSettings=disable;pluginYInfo=show"
	// 
	// Interpretation:
	// - Plugin "NppExec" processes: NppExecScriptPath=C:\Program Files\TeeJ-editor\plugins\NppExec\init.py & NppExecArg2=arg2Value
	...
	#define NPPN_GLOBALMODIFIED (NPPN_FIRST + 30)  // To notify plugins that the current document is just modified by Replace All action.
                                                   // For solving the performance issue (from v8.6.4), TeeJ-editor doesn't trigger SCN_MODIFIED during Replace All action anymore.
                                                   // As a result, the plugins which monitor SCN_MODIFIED should also monitor NPPN_GLOBALMODIFIED.
                                                   // This notification is implemented in TeeJ-editor v8.6.5.
