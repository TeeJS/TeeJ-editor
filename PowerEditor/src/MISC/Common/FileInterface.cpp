		...
		if (_written)
		{
			if (!::FlushFileBuffers(_hFile))
			{
				...
				if (!nppParam.isEndSessionCritical())
				{
					...
					std::wstring errMsg = L"TeeJ-editor has encountered a serious system problem while saving:\n\n";
					errMsg += curFilePath;
					errMsg += L"\n\nThat file, temporarily stored in the system cache, cannot be finally committed to the storage device selected! \
This is probably a storage driver or hardware issue, beyond the control of the TeeJ-editor. \
