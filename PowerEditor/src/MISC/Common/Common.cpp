...
std::string getFileContent(const wchar_t* file2read, bool* pbFailed)
{
	...
	catch ([[maybe_unused]] const std::bad_alloc& ex)
	{
		...
		::MessageBoxW(NULL, L"std::bad_alloc exception caught!\n\nProbably not enough contiguous memory to complete the operation.",
			L"TeeJ-editor - getFileContent", MB_OK | MB_ICONWARNING | MB_APPLMODAL);
	}
	catch (...)
	{
		if (pbFailed)
...
bool isAssoCommandExisting(LPCWSTR FullPathName)
{
	...
	if (isFileExisting)
	{
		...
		isAssoCmdExist = (hres == S_OK)                  // check if association exist and no error
			&& (wcsstr(buffer, L"TeeJ-editor.exe")) == NULL;   // check association with TeeJ-editor
...
bool toggleReadOnlyFlagFromFileAttributes(const wchar_t* fileFullPath, bool& isChangedToReadOnly)
{
	...
	else
	{
		...
		{
			// try to set elevated
			// (TeeJ-editor.exe #UAC-SETFILEATTRIBUTES# attrib_flags_number_str dest_file_path)
...
bool isUnsupportedFileName(const wstring& fileName)
{
	...
	// until the TeeJ-editor (and its plugins) will not be prepared for filenames longer than the MAX_PATH,
	// we have to limit also the maximum supported length below
	if ((fileName.size() > 0) && (fileName.size() < MAX_PATH))
	{
		// possible raw filenames can contain space(s) or dot(s) at its end (e.g. "\\?\C:\file."), but the TeeJ-editor advanced
		// Open/SaveAs IFileOpenDialog/IFileSaveDialog COM-interface based dialogs currently do not handle this well
		// (but e.g. direct TeeJ-editor Ctrl+S works ok even with these filenames)
		// 
...
bool isWindowVisibleOnAnyMonitor(const RECT& rectWndIn)
{
	...
	// get scaled Virtual Screen size (scaled coordinates are saved by the TeeJ-editor into config.xml)
...
DWORD invokeNppUacOp(const std::wstring& strCmdLineParams)
{
	...
	// wait for the elevated TeeJ-editor process to finish
