...
void Buffer::updateTimeStamp()
{
	...
	if (res == -1 || res == 1)
	// (res == -1) => timeStampLive is later, it means the file has been modified outside of TeeJ-editor - usual case
	// 
	// (res == 1) => timeStampLive (get directly from the file on disk) is earlier than buffer's timestamp - unusual case
	//               It can happen when user copies a backup of editing file somewhere-else firstly, then modifies the editing file in TeeJ-editor and saves it.
	//               Now user copies the backup back to erase the modified editing file outside TeeJ-editor (via Explorer).
	{
		NppParameters& nppParam = NppParameters::getInstance();
		if (nppParam.doNppLogNetworkDriveIssue())
		{
...
bool Buffer::checkFileState() // returns true if the status has been changed (it can change into DOC_REGULAR too). false otherwise
{
	...
	else if (bGetFileAttributesExSucceeded)
	{
		...
		if (res == -1 || res == 1)
			// (res == -1) => attributes.ftLastWriteTime is later, it means the file has been modified outside of TeeJ-editor - usual case
			// 
...
bool FileManager::deleteFile(BufferID id)
{
	...
	fileOpStruct.fFlags = FOF_ALLOWUNDO | FOF_NOCONFIRMATION; // FOF_NOCONFIRMATION - prevent possible redundant shell-dlg (TeeJ-editor uses its own delete-confirmation dlg)
...
bool FileManager::moveFile(BufferID id, const wchar_t* newFileName)
{
	...
	if (!::MoveFileExW(fileNamePath, newFileName, MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED | MOVEFILE_WRITE_THROUGH))
	{
		...
		// (TeeJ-editor.exe #UAC-MOVEFILE# original_file_path new_file_path)
...
For existing file (c:\tmp\foo.h)
	...
	- Close
	In the current session, TeeJ-editor
...
SavingStatus FileManager::saveBuffer(BufferID id, const wchar_t* filename, bool isCopy)
{
	...
	if (!strTempFile.empty())
	{
		...
		// (TeeJ-editor.exe #UAC-SAVE# temp_file_path dest_file_path)
...
bool FileManager::loadFileData(Document doc, int64_t fileSize, const wchar_t * filename, char* data, Utf8_16_Read * unicodeConvertor, LoadedFileFormat& fileFormat)
{
	...
	if (bufferSizeRequested > INT_MAX)
	{
		...
		if (nppParam.archType() == IMAGE_FILE_MACHINE_I386)
		{
			pNativeSpeaker->messageBox("FileTooBigToOpen",
				_pNotepadPlus->_pEditView->getHSelf(),
				L"File is too big to be opened by TeeJ-editor",
				L"File size problem",
				MB_OK | MB_APPLMODAL);
	...
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
		...
		{
			case SC_STATUS_OK:
				// either the Scintilla doesn't catch this exception or the error is in the TeeJ-editor code, report the exception anyway
#if defined(__GNUC__)
			...
			case SC_STATUS_BADALLOC:
			{
				pNativeSpeaker->messageBox("FileMemoryAllocationFailed",
					_pNotepadPlus->_pEditView->getHSelf(),
					L"There is probably not enough contiguous free memory for the file being loaded by TeeJ-editor.",
...
bool FileManager::createEmptyFile(const wchar_t* path)
{
	...
	if (!file.isOpened())
	{
		...
		// (TeeJ-editor.exe #UAC-CREATEEMPTYFILE# new_empty_file_path)
